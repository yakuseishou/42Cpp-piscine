...
/*   Updated: 2020/01/16 14:59:42 by kchen2           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
FragTrap::FragTrap(std::string name) : _hp(100), 
    ...
    std::cout << "As lightning falls onto an metal box, a sleeping robot " << _name << " is awoken!" << std::endl;
}

FragTrap::~FragTrap() {
    std::cout << _name << " got its battery knocked out its body and ceased to function..." << std::endl;
}
...
void    FragTrap::takeDamage(unsigned int amount)
{
    ...
    if (amount >= _adr)
        dmg = amount - _adr;
    std::cout << _name << " takes <" << dmg << "> amount of dmg, ";
    if (_hp < dmg) {
        _hp = 0;
        std::cout << _name << " is knocked down" << std::endl;
    }
    else {
        _hp -= dmg;
        std::cout << _name << " remains with " << _hp << "of HP" << std::endl;
    }
}
...
{
    _hp += amount;
    std::cout << "Kon Kon kinn... as tools hammering away at " << _name << "'s body, ";
    std::cout << _name << " regain " << amount << " of HP!" << std::endl;
    if (_hp > _mHP)
        _hp = _mHP;
}
